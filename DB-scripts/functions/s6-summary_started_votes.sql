...
CREATE OR REPLACE FUNCTION summary_started_votes(user_id INT) 
RETURNS TEXT
NOT DETERMINISTIC
BEGIN 
    ...
    DECLARE votes CURSOR FOR 
        SELECT pro_id_nb, pro_title_vc, vot_start_date, vot_round_nb
        FROM proposal
        ...
        AND ((CURDATE() <= vot_start_date AND vot_start_date <= CURDATE() + INTERVAL 7 DAY));
    ...
    fetchvotes: LOOP
        FETCH votes INTO proposal_id, proposal_title, vote_start, vote_round;
