<template>
    ...
    <main class="members">
        <div class="members__actions-bar">
            ...
            <button
                ...
                @click="roleUpdateValidationModal = true"
            >
        ...
        <div class="members__list">
            <div v-for="(member, key) in members" class="member-card">
                <p>
                    <b>{{ member['USR_firstname_VC'] }}</b> {{ member['USR_lastname_VC'] }}
                </p>
                <div>
                    ...
                    <select name="role-selector" @change="changeRole(member, $event.target.value)">
                        ...
                        <option
                            v-for="role in roles"
                            :key="role['ROL_id_NB']"
                            :value="role['ROL_id_NB']"
                            :disabled="role['ROL_id_NB'] === member['MEM_role_NB']"
                        >
                            {{ role['ROL_label_VC'] }}
                        </option>
                    </select>
        ...
        <template #body>
            ...
            <ul class="update-role">
                <li v-for="member in changedUser">
                    ...
                    devient
                    <b>{{ roles.find((r) => r['ROL_id_NB'] == changedRole[member['USR_id_NB']])['ROL_label_VC'] }}</b>
                </li>
            </ul>
        </template>
    </Modal>

    <Toast name="administratorMissing" :type="1" :time="5" :loader="true" class="toast">
        Impossible de se retrograder
    </Toast>

    <Toast name="roleUpdated" :type="3" :time="5" :loader="true" class="toast"> Modifications appliquées </Toast>
    ...
    <Toast name="impossibleToExcludeUser" :type="1" :time="5" :loader="true" class="toast">
        Impossible de supprimer l'utilisateur
    </Toast>

    <Toast name="userHasBeenExcluded" :type="3" :time="5" :loader="true" class="toast"> Utilisateur exclu </Toast>
    ...
    >
        ...
        <template #body>
            <TextArea
                ...
                :rules="[(v) => Boolean(v) || 'Veuillez entrez des emails']"
                :rows="10"
            >
                Entrez des emails
            </TextArea>
    ...
    <Toast name="notEveryInvitations" :type="2" :time="5" :loader="true" class="toast">
        {{ nbNotSended > 1 ? nbNotSended + " invitations n'ont pas été envoyées" : "1 invitation n'a pas été envoyée" }}
    </Toast>

    <Toast name="allInvitations" :type="3" :time="5" :loader="true" class="toast"> Envoyées ! </Toast>
</template>
...
const excludeUser = async () => {
    try {
        await $fetch(
            `${config.public.baseUrl}/communities/${route.params.id}/exclude/${userToExclude.value['USR_id_NB']}`,
            {
                method: 'POST',
                credentials: 'include',
                body: changedRole.value,
