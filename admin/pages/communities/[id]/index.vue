    ...
    <main class="community">
        <section class="community__lateral">
            <div class="community__recap-budget">
                ...
                <p>
                    <b>Budget Utilisé : </b>
                    {{ formatNumber(budget['CMY_used_budget_NB']) }} €
                </p>
                <p>
                    <b>Frais fixes :</b>
            ...
            <div class="community__recap-themes">
                ...
                <div>
                    ...
                    <p v-if="budget['CMY_budget_theme_NB']?.length === 0">
                        ...
                        <span class="underline pointer" @click="addThemeModal = true">ajouter un</span>.
                    </p>
                </div>
            </div>
        </section>
        <section class="community__main">
            <div class="community__adopted">
                ...
                <div>
                    <NuxtLink
                        :to="`/proposals/${proposal['PRO_id_NB']}`"
                        v-for="proposal in adopted"
                        class="proposal-card proposal-card"
                    >
                        <div>
                            <p class="proposal-card__theme">
                                {{ proposal['PRO_theme_VC'] }}
                            </p>
                        ...
                        <div>
                            <p>
                                {{ proposal['PRO_like_NB'] + proposal['PRO_love_NB'] }}/{{
                                    proposal['PRO_dislike_NB'] + proposal['PRO_hate_NB']
                                }}
                            </p>
                            <p>
                ...
                <div>
                    <div v-for="(proposal, key) in voted" class="proposal-card__wrapper">
                        ...
                        <NuxtLink
                            class="proposal-card"
                            :class="{
                                'proposal-card--action': best == proposal['PRO_id_NB'],
                            }"
                            :to="`/proposals/${proposal['PRO_id_NB']}`"
                        >
                            <div>
                        ...
                        <div
                            ...
                            class="proposal-card__btns"
                            :class="{
                                'proposal-card__btns--action': best == proposal['PRO_id_NB'],
                            }"
                        >
                            <button class="btn btn--small" @click="approveProposal(true, proposal['PRO_id_NB'])">
                                Adopter
                            </button>
                            <button class="btn btn--small" @click="approveProposal(false, proposal['PRO_id_NB'])">
                                Refuser
                            </button>
                        </div>
                    </div>
        ...
        <section class="community__actions">
            <div class="community__actions__btns">
                <NuxtLink
                    v-if="role['MEM_role_NB'] == ADMIN || role['MEM_role_NB'] == DECIDER"
                    class="btn btn--small"
                    :to="`/communities/${$route.params.id}/budget`"
                    >Détails du budget</NuxtLink
                >
                <button
                    v-if="role['MEM_role_NB'] == ADMIN || role['MEM_role_NB'] == DECIDER"
                    class="btn btn--small"
                    @click="addThemeModal = true"
                >
                    Ajouter un thème
                </button>
                <NuxtLink class="btn btn--small" :to="`/communities/${$route.params.id}/proposals`"
                    >Voir toutes les propositions</NuxtLink
                >
                <NuxtLink
                    v-if="role['MEM_role_NB'] == ADMIN"
                    class="btn btn--small"
                    :to="`/communities/${$route.params.id}/edit`"
                >
                    Modifier le groupe
                </NuxtLink>
                <NuxtLink
                    v-if="role['MEM_role_NB'] == ADMIN"
                    class="btn btn--small"
                    :to="`/communities/${$route.params.id}/members`"
                >
                    Gérer les membres</NuxtLink
                >
                <NuxtLink
                    v-if="role['MEM_role_NB'] == ADMIN || role['MEM_role_NB'] == MODERATOR"
                    class="btn btn--small"
                    :to="`/communities/${$route.params.id}/moderation`"
                >
                    Accéder aux outils de modérations</NuxtLink
                >
                <NuxtLink class="btn btn--small" :to="`${config.public.appUrl}/community/${$route.params.id}`">
                    Retourner au groupe</NuxtLink
                >
            </div>
    ...
    >
        ...
        <template #body>
            ...
            <p class="legende mt20">
                Le budget défini pour le thème sera ajouté pour la période en cours ({{ new Date().getFullYear() }}).
            </p>
        </template>
    </Modal>
...
const algorithm = async () => {
    try {
        const response = await $fetch(
            `${config.public.baseUrl}/communities/${route.params.id}/algo?period=${period.value}`,
            {
                method: 'GET',
                credentials: 'include',
            }
