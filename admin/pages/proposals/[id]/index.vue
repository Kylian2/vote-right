<template>
    ...
    <NuxtLink v-else-if="community" class="back" :to="`/communities/${community['CMY_id_NB']}`"
        >Retour au groupe</NuxtLink
    >
    ...
    <main class="proposal">
        <section class="section-1">
            <div v-if="initiator" class="proposal__initiator">
                <p class="profil-initials">
                    {{ getInitials(initiator['USR_firstname_VC'], initiator['USR_lastname_VC']) }}
                </p>
                <div>
                    <p>
            ...
            <div v-if="proposal['PRO_status_VC'] != 'En cours'">
                <p>La proposition a été {{ proposal['PRO_status_VC']?.toLowerCase() }}.</p>
            </div>
            <div v-if="proposal" class="proposal__informations">
                <p v-if="proposal['PRO_theme_VC']">
                    <img src="/images/icons/theme.svg" alt="icons-theme" />{{ proposal['PRO_theme_VC'] }}
                </p>
                <div v-if="budgetTheme">
                    ...
                    <p
                        :class="{
                            error:
                                proposal['PRO_budget_NB'] >
                                budgetTheme['BUT_amount_NB'] -
                                    (proposal['PRO_status_VC'] == 'Validée' ? 0 : budgetTheme['BUT_used_budget_NB']),
                        }"
                    >
                    ...
                    <p
                        v-if="role['MEM_role_NB'] == ADMIN || role['MEM_role_NB'] == DECIDER"
                        class="edit"
                ...
                <p v-if="proposal['PRO_location_VC']">
                    <img src="/images/icons/location.svg" alt="icons-theme" />{{ proposal['PRO_location_VC'] }}
                </p>
                <p v-if="proposal['PRO_period_YEAR']">
                    <img src="/images/icons/date.svg" alt="icons-theme" />{{ proposal['PRO_period_YEAR'] }}
                </p>
            </div>
            ...
            <div class="proposal__opinions" v-if="reactions">
                <h3>Avis</h3>
                <p>
                    {{ reactions['nblove'] + reactions['nblike'] }} aiment /
                    {{ reactions['nbhate'] + reactions['nbdislike'] }} n'aiment pas
                </p>
            </div>
            <div class="proposal__actions">
                <NuxtLink
                    :to="`/proposals/${$route.params.id}/votes`"
                    class="btn btn--small"
                    v-if="role['MEM_role_NB'] == ADMIN || role['MEM_role_NB'] == ASSESSOR"
                    >Plannification du vote</NuxtLink
                >
                <div
                    v-if="
                        proposal['PRO_status_VC'] === 'En cours' &&
                        votesAreFinished &&
                        budgetTheme &&
                        (role['MEM_role_NB'] == ADMIN || role['MEM_role_NB'] == DECIDER)
                    "
                >
                    <button
                        class="btn btn--small btn--full"
                        @click="approveProposal(true)"
                        :disabled="
                            proposal['PRO_budget_NB'] > budgetTheme['BUT_amount_NB'] - budgetTheme['BUT_used_budget_NB']
                        "
                    >
                        Adopter
                    </button>
                ...
                <button class="btn btn--small delete" @click="deleteModal = true" v-if="role['MEM_role_NB'] == ADMIN">
        ...
        <section class="section-2">
            ...
            <div class="proposal__formal">
                ...
                <p v-if="formalRequest['PRO_request_count_NB'] > 1">
                    Il y a {{ formalRequest['PRO_request_count_NB'] }} demandes
                </p>
            </div>
            <section class="vote-section">
                <div class="vote__results" v-if="results.length > 0 && currentVote" v-for="(result, key) in results">
                    <h3>
                        {{
                            currentVote['VOT_type_NB'] === 1 || currentVote['VOT_type_NB'] === 2
                                ? 'Résultat'
                                : `Résultat - Tour ${key + 1}`
                        }}
                    </h3>
                    <div class="vote__results__container">
                        <div v-for="(possibility, key) in result" class="result">
                            ...
                            <div class="result__stat">
                                <span
                                    class="result__stat__bar"
                                    :style="{
                                        width: `${possibility['POS_percentNbVotes_NB']}%`,
                                    }"
                                ></span>
                                <span class="result__stat__value legende"
                                    >{{ possibility['POS_percentNbVotes_NB'] }}%</span
                                >
                            </div>
                        </div>
                    </div>
                    <p
                        class="legende"
                        v-if="votes.filter((v) => v['VOT_round_NB'] == key + 1)[0]['VOT_valid_BOOL'] == 'null'"
                    >
                        Le vote n'a pas encore été validé
                    </p>
                    <p class="legende" v-else>
                        {{
                            votes.filter((v) => v['VOT_round_NB'] == key + 1)[0]['VOT_valid_BOOL']
                                ? 'Le vote a été validé'
                                : 'Le vote a été refusé'
                        }}
                    </p>
                    <div
                        class="vote__results__actions"
                        v-if="
                            votes.filter((v) => v['VOT_round_NB'] == key + 1)[0]['VOT_valid_BOOL'] === 'null' &&
                            role['MEM_role_NB'] == ASSESSOR
                        "
                    >
    ...
    >
        ...
        <template #body>
            ...
            <Input
                type="text"
                :placeholder="proposal['PRO_title_VC'].toUpperCase()"
                class="inline-input"
                name="deleteValidation"
                :rules="[(v) => v === proposal['PRO_title_VC'].toUpperCase() || '']"
                >Réécrivez :
            </Input>
        </template>
    </Modal>
    ...
    >
        ...
        <template #body>
            <div class="budget-infos-container">
                ...
                <p>
                    Budget utilisé : {{ formatNumber(budget['CMY_used_budget_NB'] + budget['CMY_fixed_fees_NB']) }} €
                    /an
                </p>
            ...
            <div class="budget-input-container">
                ...
                <InputNumber
                    type="text"
                    :placeholder="proposal['PRO_budget_NB'] + ''"
                    :min="0"
...
const discussionDate = computed(() => {
    ...
    const adjustedDate = new Date(
        creationDate.setDate(creationDate.getDate() + proposal.value['PRO_discussion_duration_NB'])
    )
    return adjustedDate
})
...
const fetchData = async () => {
    try {
        ...
        const bud = await $fetch(
            `${config.public.baseUrl}/communities/${proposal.value['PRO_community_NB']}/budget?period=${proposal.value['PRO_period_YEAR']}`,
            {
                credentials: 'include',
            }
        )
    ...
    try {
        ...
        if (votes.value.length > 0) {
            ...
            for (let i = 0; i < votes.value.length; i++) {
                let rmt = calculateTimeRemaining(
                    new Date(),
                    new Date(votes.value[i]['VOT_end_DATE']?.replace(' ', 'T'))
                )
                if (rmt === 0) {
