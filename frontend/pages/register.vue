<template>
        ...
        <div class="registration">
            ...
            <form class="registration__form">
                ...
                <Input
                    ...
                    required
                    :rules="[(v) => Boolean(v) || 'Un nom est requis', (v) => v.length < 50 || 'Le nom est trop long']"
                    >Nom</Input
                >
                <Input
                    v-if="step === 1"
                ...
                <div v-if="step === 2" class="inline-input">
                    <Input
                        class="inline-input__first"
                        type="text"
                        name="city"
                        placeholder="ex: Paris"
                        required
                        :rules="[(v) => Boolean(v) || 'Une ville est requise']"
                        >Ville</Input
                    >
                    <Input
                        class="inline-input__second"
                        type="text"
                        name="zipcode"
                ...
                <Button
                    ...
                    @click="sendCode()"
                    >Recevoir le code de verification</Button
                >
                ...
                <Button
                    ...
                    "
                    >Commencer l'inscription</Button
                >
                <div v-if="stepOneIsValid" class="registration__button-conteneur">
                    <Button
                        v-if="step === 2"
                        :disabled="!stepOneIsValid || !stepTwoIsValid"
                        formmethod="dialog"
                        class="btn btn--full"
                        @click="handleForm()"
                        >Terminer l'inscription</Button
                    >
                    <Button
                        v-if="step === 2"
                        type="button"
    ...
    <Toast name="sameEmail" :type="1" :time="5" :loader="true" class="toast"> Email déjà utilisé </Toast>

    <Toast name="incorrectCode" :type="1" :time="5" :loader="true" class="toast"> Code incorrect </Toast>
...
const sendCode = async () => {
    try {
        const response = await $fetch(`${config.public.baseUrl}/code/verification`, {
            method: 'POST',
            body: {
                email: email.value,
            },
