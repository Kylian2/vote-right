<template>
    <Header
        type="logged"
        :color="community && community['CMY_color_VC'] ? community['CMY_color_VC'].slice(-6) : '000000'"
    ></Header>
            ...
            <div class="new-proposal__form__input-container">
                <div>
                    <Input
                        type="text"
                        name="proposalTitle"
                        placeholder="Donnez un titre à la proposition"
                        required
                        :rules="[(v) => Boolean(v) || 'Un titre est requis']"
                        >Titre</Input
                    >
                    <TextArea
                ...
                <div>
                    <InputNumber
                        ...
                        :min="1900"
                        :max="2099"
                        :placeholder="`Par défaut : ${new Date().getFullYear()}`"
                        :rules="[(v) => !v || (v >= 1900 && v <= 2099) || 'La valeur doit être entre 1900 et 2099']"
                        >Année</InputNumber
                    >
                    <Input type="text" name="proposalLocation" placeholder="Ex: 12 avenue du blé"
                        >Indiquez une localisation</Input
                    >
                </div>
            </div>
    ...
    try {
        const response = await $fetch(`${config.public.baseUrl}/proposals`, {
            ...
            body: {
                ...
                theme: theme.value,
                year: yearValid.value && year.value !== '' ? year.value : new Date().getFullYear(),
            },
            credentials: 'include',
        })
