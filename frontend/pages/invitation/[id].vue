<template>
    ...
    <main>
        <div v-if="invitationUnavailable">
            <h3 style="text-align: center; margin-top: 100px; margin-bottom: 100px">
                Cette invitation est inaccessible
            </h3>
        </div>
        ...
        <div v-else class="invitation">
            <div
                ...
                v-if="community"
                :style="{ background: `url('${config.public.baseUrl}/${community['CMY_image_VC']}') 0% 15% / cover` }"
            ></div>
            ...
            <div v-else class="invitation__show">
                <div class="invitation__show__content" v-if="invitation && community">
                    <h2>
                        {{ invitation['INV_sender_firstname_VC'] }} {{ invitation['INV_sender_lastname_VC'] }}
                ...
                <div class="invitation__show__community">
                    <div class="invitation__show__community__themes">
                        ...
                        <div
                            class="invitation__show__community__themes__list"
                            v-if="communityThemes && communityThemes.length"
                        >
                ...
                <div class="invitation__show__actions">
                    ...
                    <div class="invitation__show__actions__button">
                        <button class="btn btn--full" :disabled="!validCode" @click="acceptInvitation()">
                            Accepter
                        </button>
                        <button class="btn btn--cancel" :disabled="!validCode" @click="rejectInvitation()">
                            Refuser
                        </button>
                    </div>
                </div>
...
const fetchData = async () => {
    try {
        ...
        const response3 = await $fetch(
            `${config.public.baseUrl}/communities/${invitation.value.INV_community_NB}/themes`,
            {
                credentials: 'include',
            }
        )
