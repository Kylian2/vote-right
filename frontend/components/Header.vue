<template>
    <header class="header--desktop" :class="{ [`color-${color}`]: color }">
        ...
        <nav v-if="type === 'logged'" class="logged">
            <div class="wrapper" :class="{ active: actif === 'accueil' }">
                <NuxtLink class="logged__link" to="/home">Accueil</NuxtLink>
            </div>
            <div class="wrapper" :class="{ active: actif === 'groupes' }">
                <NuxtLink class="logged__link" to="/communities">Groupes</NuxtLink>
            </div>
        </nav>
    ...
    <Modal
        ...
        cancel-text="Quitter"
        :disable-valid="
            (((!allowedInformationChanges && !allowedPasswordChange) || !settingsIsValid) && MyInformationSection) ||
            (!notificationChanged && !MyInformationSection)
        "
        :before-cancel="
            () => {
                beforeCancel()
    ...
    >
        ...
        <template #subtitles>
            <div class="header-settings-modal__section">
                <p
                    @click=";(MyInformationSection = true), sectionChanged()"
                    :style="{ fontWeight: MyInformationSection ? 'bold' : 'normal' }"
                >
                    Mes informations
                </p>
                <p
                    @click=";(MyInformationSection = false), sectionChanged()"
                    :style="{ fontWeight: !MyInformationSection ? 'bold' : 'normal' }"
                >
                    Notifications
                </p>
        ...
        <template #body>
                ...
                <div v-if="MyInformationSection">
                    <div class="container-checkbox">
                        <div class="form-check">
                            <input
                                @click="editInformationChange"
                                type="checkbox"
                                id="editInformation"
                                v-model="allowedInformationChanges"
                            />
                            <label for="editInformation"> Modifier vos informations </label>
                        </div>
                    </div>
                    <div class="header-settings-modal__allowedChanges">
                        <div class="container-checkbox">
                            <div class="form-check">
                                <input
                                    @click="editPasswordChange"
                                    type="checkbox"
                                    id="editPassword"
                                    v-model="allowedPasswordChange"
                                />
                                <label for="editPassword"> Modifier votre mot de passe </label>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <form v-if="MyInformationSection" class="header-settings-modal__settings-list">
                <div>
                    <p v-if="!allowedInformationChanges">
                        Date de naissance :<span>{{
                            user['USR_birthdate_DATE'] ?? "Pas d'information enregistrée"
                        }}</span>
                    </p>
                    <Input
                        v-else
                ...
                <div>
                    <p v-if="!allowedInformationChanges">
                        Adresse :<span>{{ user['USR_address_VC'] ?? "Pas d'information enregistrée" }}</span>
                    </p>
                    <Input
                        v-else
                        type="text"
                ...
                <div>
                    <p v-if="!allowedInformationChanges">
                        Code postal :<span>{{ user['USR_zipcode_CH'] ?? "Pas d'information enregistrée" }}</span>
                    </p>
                    <Input
                        v-else
                        type="text"
                ...
                <div>
                    <p v-if="!allowedInformationChanges">
                        Email :<span>{{ user['USR_email_VC'] }}</span>
                    </p>
                    <Input
                        v-else
                        type="email"
            ...
            <div v-else class="header-settings-modal__settings-list">
                <div class="container-checkbox">
                    <div class="form-check">
                        <input
                            @change="notificationChanged = true"
                            type="checkbox"
                            id="newProposal"
                            :value="`newProposal`"
    ...
    <Toast name="settingsValid" :type="3" :time="5" :loader="true" class="toast">
...
const settingsIsValid = computed(() => {
    if (allowedInformationChanges.value && allowedPasswordChange.value) {
        return (
            birthdateValid.value && addressValid.value && zipcodeValid.value && emailValid.value && passwordValid.value
        )
    }
