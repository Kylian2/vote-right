	...
	<div class="emoji-picker flex align-start">
		<div class="base-2">
			...
			<div class="emoji-picker__container" ref="pickerContainer">
				<button type="button" class="emoji-picker__btn" @click="togglePicker" :class="{ emoji: selectedEmoji }">
					{{ selectedEmoji || placeholder || "Choisir" }}
				</button>
			</div>
		</div>
		<div v-if="showPicker" class="emoji-picker__picker">
			<div v-for="emoji in emojisList" :key="emoji" class="emoji-picker__emoji" @click="selectEmoji(emoji)">
				{{ emoji.emoji }}
			</div>
		</div>
